...
num_repetitions = 3
max_retries = 2
time_delay_between_retries = 1
time_delay_between_inferences = 1
...
data = pd.read_excel(config['dataset_excel_path_test']).head(num_records)
        ...
        for i in range(1, num_repetitions + 1):
            ...
            while retries < max_retries:
                ...
                completion = client.chat.completions.create(model=model_name,messages=[
                    {"role": "system", "content": "T2S is a Text to SQL converter"},
                    {"role": "user", "content": prompt_completed}])
                end_time = time.perf_counter()
